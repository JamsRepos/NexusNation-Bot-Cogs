        ...
        if auto_lyrics is True:
            ...
            async with notify_channel.typing():
                try:
                    ...
                    paged_embeds = []
                    paged_content = [p for p in pagify(results, escape_mass_mentions=True, page_length=700)]
                    ...
                    for index, page in enumerate(paged_content, start=1):
                        ...
                        e.set_footer(text=f"Page {index}/{len(paged_content)}")
        ...
        try:
            ...
            for index, page in enumerate(paged_content, start=1):
                e = discord.Embed(title='Lyrics for __{}__'.format(artistsong), description=page,
                    colour=await self.bot.get_embed_color(ctx.channel))
                e.set_footer(text=f"Requested by {ctx.message.author} | Page {index}/{len(paged_content)}")
                paged_embeds.append(e)
