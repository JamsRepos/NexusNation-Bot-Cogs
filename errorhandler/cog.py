...
#embed_error = discord.Embed(title="Oops!", description=f"Error in command `{ctx.command.qualified_name}`. I've send this off!")

#default_reply = "await ctx.send(f\"`Error in command '{ctx.command.qualified_name}'. Check your console or logs for details.`\")"
default_reply1 = "await ctx.send(embed=embed_error)"
GLOBAL_DEFAULT = {'response': default_reply1}
...
class ErrorHandler(commands.Cog):
    ...
    async def on_command_error(self, ctx, error, unhandled_by_cog=False):
        if isinstance(error, commands.CommandInvokeError):
            ...
            embed_error = discord.Embed(title="Oops! An error has occured:", 
                description=f'```py\nError in command "{ctx.command.qualified_name}"\nThis error has been received by the developers.```'
                + f'\nIf this error consists, please report it by using **`{ctx.prefix}contact`**', colour=(await ctx.embed_colour()
                )
            )

            eh_cog = ctx.bot.get_cog("ErrorHandler")
            if eh_cog._eval_string is None:
                eh_cog._eval_string = await ctx.send(embed=embed_error)
