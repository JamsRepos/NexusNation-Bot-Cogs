...
class HelpMenu(commands.Cog):
    ...
    async def help(self, ctx, *, command=None):
        ...
        utilitycommands = """`starboard` `customcom`  `filter` `filterset` `autorole` `alias` `google` `away` `toggleaway` `gif` `imgtfy` `imgur` `covid` `color` `autoroom` `autoroomset` `infochannel` `infochannelset`"""
