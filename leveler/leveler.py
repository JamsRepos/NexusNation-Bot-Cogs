...
class Leveler(commands.Cog):
    ...
    def make_full_profile(self, avatar_data, user, xp, nxp, lvl, minone, elo, ldb, desc, bg=None):
        ...
        usercolor = (248, 46, 61)  # user.color.to_rgb()
        if user.status == discord.Status.online:
            statuscolor = (67, 181, 129, 255)
        elif user.status == discord.Status.dnd:
            statuscolor = (240, 71, 71, 255)
        elif user.status == discord.Status.idle:
            statuscolor = (250, 166, 26, 255)
        else:
            statuscolor = (116, 127, 141, 255)
        
        aviholder = self.add_corners(Image.new("RGBA", (140, 140), statuscolor), 0)
        #nameplate = self.add_corners(Image.new("RGBA", (180, 85), (10, 10, 10, 255)), 0)
        xptot = self.add_corners(Image.new("RGBA", (310, 20), (56, 56, 56, 255)), 0)
        img.paste(aviholder, (10, 10), aviholder)
        #img.paste(nameplate, (155, 35), nameplate)
        img.paste(xptot, (15, 340), xptot)

        fontpath = str(bundled_data_path(self) / "Biko_Regular.otf")
        ...
        xpbar = self.add_corners(Image.new("RGBA", (b_offset, 20), usercolor), 0)
        ...
        draw.text((165, 343), f"{lprc}%", fill="white", font=font1)
        draw.text((100, 365), (prog_str + f" {xp}/{nxp}"), fill=usercolor, font=font1)
        ...
        draw.text((162, 30), f"{nick}", fill="white", font=font2)
        draw.text((162, 60), f"{user.name}#{user.discriminator}", fill="white", font=font1)
        if user.activity != None:
            draw.text((162, 90), f"Playing:\n{user.activity.name}", fill="white", font=font1)
        margin = 162
        offset = 70
        count = 0
        temp = BytesIO()
    ...
    async def profile(self, ctx, user: discord.Member = None):
        """Show your profile progress. Default to yourself."""
        if user is None:
            user = ctx.author
